...
class Trainer:
    def __init__(self, configs):
        ...
        torch.manual_seed(5)
        self.network = ConvTTLSTMNet(configs.order, configs.steps, configs.ranks, configs.kernel_size,
    ...
    def train_once(self, sst, nino_true, ratio):
        sst_pred, nino_pred = self.network(sst.float(), teacher_forcing=True, 
    ...
    def test(self, dataloader_test):
        ...
        with torch.no_grad():
            for sst, _ in dataloader_test:
                sst, nino = self.network(sst.float(), train=False)
